...
| `scripts/generate_signal_window_graphs.py` | Signal/window/STFT visualization generation | matplotlib docs, SciPy signal docs | matplotlib license, BSD-3-Clause (SciPy) | Uses documented plotting and DSP APIs to generate educational figures from synthetic signals. |
| `scripts/compare_time_stretch_kpis.py` | Real-input KPI computation + method comparison | SciPy STFT/resample docs, ffmpeg docs, librosa docs | BSD-3-Clause (SciPy), LGPL/GPL dependent (ffmpeg), ISC (librosa) | Uses public APIs and original orchestration logic for comparative KPI reporting. |
