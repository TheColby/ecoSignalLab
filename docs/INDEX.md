# Documentation Index

Quick links: [Getting Started](GETTING_STARTED.md) | [Task Recipes](TASK_RECIPES.md) | [Troubleshooting](TROUBLESHOOTING.md) | [Schema](SCHEMA.md) | [Metrics](METRICS_REFERENCE.md) | [References](REFERENCES.md)

## Start Here

- [Getting Started](GETTING_STARTED.md)
- [Task Recipes (Beginner)](TASK_RECIPES.md)
- [Troubleshooting](TROUBLESHOOTING.md)
- [Roadmap](../ROADMAP.md)
- [Announcement FAQ](ANNOUNCEMENT_FAQ.md)
- [Glossary](GLOSSARY.md)
- [Shard Workflows](SHARD_WORKFLOWS.md)
- [Man Pages](MANPAGES.md)

## Recipe Index by Device Type

- Phone/laptop built-in microphone:
  - [Task Recipes: Recipe 1](TASK_RECIPES.md#recipe-1-analyze-one-file-and-generate-plots)
  - [Task Recipes: Recipe 2](TASK_RECIPES.md#recipe-2-extract-the-single-most-novel-moment)
- USB stereo interface / handheld stereo recorder:
  - [Task Recipes: Recipe 1](TASK_RECIPES.md#recipe-1-analyze-one-file-and-generate-plots)
  - [Task Recipes: Recipe 5](TASK_RECIPES.md#recipe-5-batch-analyze-a-folder)
  - [Task Recipes: Recipe 8](TASK_RECIPES.md#recipe-8-find-the-most-similar-files-in-a-folder)
- Ambisonic B-format recorder (FOA):
  - [Task Recipes: Recipe 1](TASK_RECIPES.md#recipe-1-analyze-one-file-and-generate-plots)
  - [Task Recipes: Recipe 3](TASK_RECIPES.md#recipe-3-extract-top-k-moments-instead-of-one)
  - [RF64 and Large Files](RF64_AND_LARGE_FILES.md)
- Multichannel array / Atmos-capable workflow:
  - [Task Recipes: Recipe 5](TASK_RECIPES.md#recipe-5-batch-analyze-a-folder)
  - [Task Recipes: Recipe 6](TASK_RECIPES.md#recipe-6-compare-architectural-variants)
  - [Schema Contract](SCHEMA.md)
- Sensor node / long-duration deployment:
  - [Task Recipes: Recipe 2](TASK_RECIPES.md#recipe-2-extract-the-single-most-novel-moment)
  - [Task Recipes: Recipe 3](TASK_RECIPES.md#recipe-3-extract-top-k-moments-instead-of-one)
  - [Task Recipes: Recipe 13](TASK_RECIPES.md#recipe-13-build-and-analyze-a-shard-manifest)
  - [Task Recipes: Recipe 14](TASK_RECIPES.md#recipe-14-find-the-top-33-most-novel-moments-in-a-ten-year-8-channel-file)
  - [Moments Extraction](MOMENTS_EXTRACTION.md)
  - [RF64 and Large Files](RF64_AND_LARGE_FILES.md)

## By Workflow

- Core analysis:
  - [Getting Started](GETTING_STARTED.md)
  - [Task Recipes](TASK_RECIPES.md)
- Novelty, moments, and anomaly:
  - [Novelty and Anomaly](NOVELTY_ANOMALY.md)
  - [Moments Extraction](MOMENTS_EXTRACTION.md)
  - [Similarity Search](SIMILARITY_SEARCH.md)
- Metrics and schema:
  - [Metrics Reference](METRICS_REFERENCE.md)
  - [Schema Contract](SCHEMA.md)
  - [Validation](VALIDATION.md)
- ML export:
  - [ML Features / FrameTable](ML_FEATURES.md)
- Large files and formats:
  - [RF64 and Large Files](RF64_AND_LARGE_FILES.md)
  - [Shard Workflows](SHARD_WORKFLOWS.md)
- Project architecture:
  - [Algorithm Index](ALGORITHM_INDEX.md)
  - [Algorithm Comparison](ALGORITHM_COMPARISON.md)
  - [Phase 1 Ecosystem and Gap Analysis](PHASE1_ECOSYSTEM_GAP_ANALYSIS.md)

## Standards, Attribution, and Release

- [References](REFERENCES.md)
- [Attribution](ATTRIBUTION.md)
- [Release Process](RELEASE.md)
- [Documentation Automation](DOCS_AUTOMATION.md)
- [Man Pages](MANPAGES.md)
