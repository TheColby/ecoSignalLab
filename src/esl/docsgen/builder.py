...
def _build_nav(rendered_pages: list[_RenderedPage], current_html: Path) -> str:
    items = []
    curr_res = current_html.resolve()
    for page in rendered_pages:
        label = page.title
        href = os.path.relpath(page.out_html, start=current_html.parent).replace("\\", "/")
        is_current = page.out_html.resolve() == curr_res
        aria_curr = ' aria-current="page"' if is_current else ""
        items.append(f'<a href="{href}"{aria_curr}>{html.escape(label)}</a>')
    return "\n".join(items)
