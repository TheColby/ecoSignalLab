...
def _ensure_visual_outline(markdown_text: str, page_title: str) -> str:
    ...
    outline = _build_auto_visual_block(markdown_text, fallback_title=page_title)
    return markdown_text.rstrip() + outline
...
def _build_nav(rendered_pages: list[_RenderedPage], current_html: Path) -> str:
    ...
    for page in rendered_pages:
        ...
        active_attr = ' class="active" aria-current="page"' if is_active else ""
        items.append(f'<a href="{href}"{active_attr}>{html.escape(label)}</a>')
    return "\n".join(items)
