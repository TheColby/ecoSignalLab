...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    nav a[aria-current="page"] {{ font-weight: bold; color: var(--ink); border-left: 2px solid var(--link); padding-left: 8px; margin-left: -10px; }}
    ...
    main {{ padding: 28px 32px 48px; outline: none; }}
...
def _build_nav(rendered_pages: list[_RenderedPage], current_html: Path) -> str:
    items = []
    current_abs = current_html.resolve()
    for page in rendered_pages:
        label = page.title
        is_current = page.out_html.resolve() == current_abs
        attr = ' aria-current="page"' if is_current else ""
